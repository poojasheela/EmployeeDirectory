...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    @Override
    public Page<EmployeeDTO> fetchPaginatedEmployees(int page, int size, String sortBy, String sortDir) {
        try {
            Sort sort = sortDir.equalsIgnoreCase("asc") ? Sort.by(sortBy).ascending() : Sort.by(sortBy).descending();
            Pageable pageable = PageRequest.of(page, size, sort);
            Page<Employee> empPage = employeeRepository.findAll(pageable);
            return empPage.map(mapper::toDTO);
        } catch (Exception e) {
            log.error("Error while paginating employees", e);
